...
% find indeces where the ratio is <= zeta && >= 1/zeta
desired_indRatio_1 = 1/zeta <= desired ./ (interferer + eps);
desired_indRatio_2 = desired ./ (interferer + eps) <= zeta;
desired_indRatio = desired_indRatio_1 .* desired_indRatio_2;

% ratio masks
ratio_mask = desired ./ (desired + interferer + eps);

% find indeces of binary
...
combined_mask = desired_keepBinary .* binary_mask + desired_indRatio .* ratio_mask;